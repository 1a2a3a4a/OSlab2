...
inc_cas(void *arg __attribute__((unused)))
{
    ...
    for (i = 0; i < INC_ITERATIONS; i++) {
        ...
        while( __sync_bool_compare_and_swap(&counter, local_counter ,local_counter + INCREMENT) == 0){
...
dec_cas(void *arg __attribute__((unused)))
{
     ...
     * variable */
    for (i = 0; i < DEC_ITERATIONS; i++) {
        while( __sync_bool_compare_and_swap(&counter, local_counter ,local_counter - DECREMENT ) == 0){
            local_counter = counter;
        }
